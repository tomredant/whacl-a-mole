...
bool SoundPlayer::playMusic() {
    ...
    const double TRACK_DURATION = 29.3; //duration of the track in seconds.
...
bool SoundPlayer::playEventSound() {
    ...
    //qDebug() << Pa_GetVersionText();
    const int SAMPLE_RATE = 44100*2;
    const double TRACK_DURATION = 0.35; //duration of the track in seconds.
    const int DURATION_BYTES = (int) (((double) (2*SAMPLE_RATE))*TRACK_DURATION); //factor 2 is due 16 bit sound.
    const int OFFSET = 0;
    PaError paError = Pa_Initialize();
    PaStream* stream = nullptr;
...
bool SoundPlayer::playWinSound() {
    ...
    //qDebug() << Pa_GetVersionText();
    const int SAMPLE_RATE = 44100*2;
    const double TRACK_DURATION = 0.75; //duration of the track in seconds.
    const int DURATION_BYTES = (int) (((double) (2*SAMPLE_RATE))*TRACK_DURATION); //factor 2 is due 16 bit sound.
    const int OFFSET = 0;
    PaError paError = Pa_Initialize();
    PaStream* stream = nullptr;
